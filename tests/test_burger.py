...
class TestBurger:
    def test_burger_init_has_no_buns_and_empty_ingredients(self):
        burger = Burger()
    ...
    def test_move_ingredient_moves_from_to(self):
        burger = Burger()
        burger.ingredients = [1, 2, 3]
        burger.move_ingredient(0, 2)
        assert burger.ingredients == [2, 3, 1]

    def test_get_price_calculates_bun_and_ingredients(self):
        burger = Burger()
        mock_bun = MagicMock(spec=Bun)
